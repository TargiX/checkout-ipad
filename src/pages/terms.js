...
const Terms = () => {
   ...
   let humanDate = today.getDate() + ' ' + today.toLocaleString('default', { month: 'long' }) + ' ' + today.getFullYear()
   ...
   const handleSubmit = async (event) => {
      ...
      params);
         ...
         setError(err.response.data.split("<pre>")[1].split('</pre>')[0])
         setLoading(false)
      } finally {
         console.log(result)
            if (result) {
   ...
   return (
      ...
      <Footer action={handleSubmit} disabled={confirmButton} bouncing={true} loading={loading} error={errorMessage} location="/emergency-contact" progress={90} />
      </>
   );
}
